...
/*
 * Generates an SPX key pair given a seed of length CRYPTO_SEEDBYTES (3*SPX_N)
                             ...
                             const unsigned char *seed)
{
    ...
    memcpy(sk, seed, CRYPTO_SEEDBYTES); /* sk = SK_SEED || SK_PRF || PUB_SEED */

    memcpy(pk, sk + 2*SPX_N, SPX_N); /* pk = PUB_SEED */

    memcpy(ctx.pub_seed, pk, SPX_N); /* ctx.pub_seed = PUB_SEED */
    memcpy(ctx.sk_seed, sk, SPX_N);  /* ctx.sk_seed = SK_SEED */
    ...
    merkle_gen_root(sk + 3*SPX_N, &ctx); /* sk += root */

    memcpy(pk + SPX_N, sk + 3*SPX_N, SPX_N); /* pk += root */
                          ...
                          const uint8_t *m, size_t mlen, const uint8_t *sk)
{
    ...
    for (i = 0; i < SPX_D; i++) {
        set_layer_addr(tree_addr, i); //layer
        set_tree_addr(tree_addr, tree);//tree

        copy_subtree_addr(wots_addr, tree_addr); 
        set_keypair_addr(wots_addr, idx_leaf); //leaf
