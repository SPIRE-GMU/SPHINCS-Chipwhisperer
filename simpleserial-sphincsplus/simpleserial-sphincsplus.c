...
{ // Sign message with private key
    ...
    unsigned char out[SPX_N]  = { 0x10 ,0x30};
...
{ // Sign message with private key
    ...
    // for (int i = 22*SPX_N; i < 23*SPX_N; ++i)
    //     putch(sig[i]);
    
    
    simpleserial_put('r', SPX_N, (uint8_t*)pk ); //
    // simpleserial_put('r', SPX_WOTS_LEN, (uint8_t*)steps ); //
    simpleserial_put('r', SPX_N, (uint8_t*)sig ); //
...
uint8_t test_merkle(uint8_t* m, uint8_t len)
{
    ...
    // for (i = 0; i < SPX_N; ++i) //not feasible to print all
    //     putch(root[i]);

    simpleserial_put('r', SPX_N, (uint8_t*)root ); //
    // simpleserial_put('r', SPX_N, sig+0*SPX_N ); //
    // simpleserial_put('r', SPX_N, sig+1*SPX_N ); //
...
uint8_t get_sig(uint8_t* m, uint8_t len)
{
    ...
    uint32_t idx =  m[1];
    simpleserial_put('r', SPX_N, sig+idx*SPX_N ); //
...
{ // Get private key
    ...
    simpleserial_put('r', SPX_N, (uint8_t*)cache ); //
    simpleserial_put('r', SPX_N, (uint8_t*)pk_root ); //
...
uint8_t test_msg(uint8_t* m, uint8_t len)
{
	/**********************************
	* Start user-specific code here. */
    ...
    ull_to_bytes(t, 8, tree);
    simpleserial_put('r', 8, (uint8_t*)t ); //
    return 0x00;
}


//to simplify the test, we add a 4-byte counter attached to the message; 
//counter should be stored in device memory inpractical case  
uint8_t test_msg_cnt(uint8_t* m, uint8_t len)
{
	/**********************************
	* Start user-specific code here. */
    unsigned char t[64];
    ...
    /* Derive the message digest and leaf index from R, PK and M. */
    trigger_high();
    // thash(root, m, len/SPX_N, &ctx, addr);
    hash_message(mhash, &tree, &idx_leaf, sphincs_sig, pk, m, len, &ctx);
...
int main(void)
{
    ...
    simpleserial_addcmd('p', 0, get_pk); //limited commend space
    ...
    //quick simpleserial test:
    simpleserial_addcmd('f', SPX_N, test_msg);
    simpleserial_addcmd('g', SPX_N+4, test_msg_cnt); //assuming 4 bytes for counter
